<xsl:stylesheet xmlns="http://www.loc.gov/MARC21/slim" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
                >
  ...
  <xsl:template name="transform-leader">
    ...
    <xsl:variable name="leader05" select="translate(substring($leader,6,1), 'o', 'c')"/>
    <xsl:variable name="leader06" select="translate(substring($leader,7,1), 'hmn', 'aor')"/>
  ...
  <xsl:template name="transform-100">
    ...
    <xsl:variable name="dest00-05" select="substring($source,3,6)"/>
    <xsl:variable name="dest06" select="translate(substring($source,9,1), 'abcdefghij', 'cdusrqmtpe')"/>
    <xsl:variable name="dest07-14" select="substring($source,10,8)"/>
    <xsl:variable name="dest15-21" select="'#######'"/>
    <xsl:variable name="dest22" select="translate(substring($source,1+17,1), 'bcadekmu', 'abjcdeg#')"/>
    <xsl:variable name="dest23-27" select="'#####'"/>
    <xsl:variable name="dest28" select="translate(substring($source,1+20,1), 'abcdefghy', 'fsllcizo#')"/>
    <xsl:variable name="dest29-32" select="'####'"/>
    <xsl:variable name="dest33" select="substring($source,1+34,1)"/>
    <xsl:variable name="dest34-37" select="'####'"/>
    <xsl:variable name="dest38" select="translate(substring($source,1+21,1), '01', '#o')"/>
